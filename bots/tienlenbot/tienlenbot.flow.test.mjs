...
test('phát tiền cho cả sòng: cả sổ được chia, và người phát lên bảng công đức', async () => {
  ...
  await withBot(async (app) => {
    ...
    assert.equal(lobby.meritPer, 1_000);
    ...
    assert.equal(after.meritMine, 100, 'một trăm nghìn là một trăm công đức');
    assert.deepEqual(after.merit.map((one) => [one.name, one.merit]), [['Thọ', 100]]);
    assert.equal(after.says, 'Đã phát 100.000 vàng cho 2 người · +100 công đức.',
...
test('phát hai lần thì cộng dồn, và sổ vẫn không đẻ ra đồng nào', async () => {
  ...
  await withBot(async (app) => {
    ...
    assert.equal(after.meritMine, 350, 'công đức cộng dồn, không phải lấy lần cuối');
